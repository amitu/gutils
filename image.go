...
// MacARGB is used to represent the image obtained from OS X screenbuffer.
type MacARGB struct {
	image.RGBA
}
...
func (m *MacARGB) At(x, y int) color.Color {
	r, g, b, a := m.RGBA.At(x, y).RGBA()
	return color.RGBA{uint8(b), uint8(g), uint8(r), uint8(a)}
}

// NewMacARGB returns a MacARGB object with given pixel data
func NewMacARGB(r image.Rectangle, stride int, pix []byte) *MacARGB {
	return &MacARGB{RGBA: image.RGBA{Pix: pix, Stride: stride, Rect: r}}
}
...
// NewWinBGRA returns a MacARGB object with given pixel data
func NewWinBGRA(r image.Rectangle, stride int, pix []byte) *MacARGB {
	return &MacARGB{RGBA: image.RGBA{Pix: pix, Stride: stride, Rect: r}}
}
